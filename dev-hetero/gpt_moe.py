...
class GPT(nn.Module):
    ...
    def update_balance_bias(self):
        ...
        Called after each optimizer step. Adjusts bias to encourage balanced routing.
        """
        ...
        for i, block in enumerate(self.transformer.h):
            if i not in self.moe_layer_indices:
                continue
            moe = block.mlp
            if moe._last_topk_indices is None:
                continue

            n_experts = self.config.n_routed_experts
            N = moe._last_topk_indices.shape[0]
            # Count how many tokens each expert got
            expert_counts = torch.zeros(n_experts, device=moe._last_topk_indices.device)
            for k in range(self.config.moe_top_k):
                expert_counts.scatter_add_(0, moe._last_topk_indices[:, k],
                                           torch.ones(N, device=moe._last_topk_indices.device))
            # Expected count per expert if balanced
            expected = N * self.config.moe_top_k / n_experts
